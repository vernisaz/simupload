...
extern crate simweb;
use simweb::{WebData, WebError, WebPage};
use std::{
    ffi::OsStr,
    fmt::Write,
    fs,
    ...
    fn main_load(&self) -> Result<String, Box<dyn std::error::Error + 'static>> {
        ...
        if let Some(dir) = data.param("dir") {
            match data.params("upFile") {
                None => Err(Box::new(WebError {
                    reason: "Err: no files".to_string(),
                    cause: None,
                })),
                Some(files) => {
                            ...
                            Err(mut err) => {
                                ...
                                match errors {
                                    None => {
                                        ...
                                        writeln! {&mut some_errors, "{file} to {dir} {err:?}"}
                                            .unwrap();
